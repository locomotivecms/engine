module Locomotive
  module PagesHelper
    ...
    #   nodes = tree.map { |page, children| Node.new(page, children) }

    #   render 'locomotive/shared/sidebar/pages', nodes: nodes, root: tree.first.first
    # end

    # class Node < Struct.new(:page, :children)

    #   extend Forwardable

    #   def_delegators :page, :_id, :index_or_not_found?, :published?, :templatized?

    #   def fold_state
    #     cookies["node-#{_id}"] != 'folded' ? 'unfolded' : 'folded'
    #   end
