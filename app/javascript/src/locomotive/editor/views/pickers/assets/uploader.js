...
class Uploader extends Component {
  ...
  handleUpload(event) {
    ...
    if (files.length != event.target.files.length)
      alert(i18n.t('views.pickers.assets.too_big'));
    ...
    return (
      <div className="editor-asset-uploader">
        ...
        {!this.state.uploading && (
          <a className="editor-asset-uploader--button" onClick={this.openDialog}>
            {i18n.t('views.pickers.assets.add')}
          </a>
        )}

        {this.state.uploading && (
          <div className="editor-asset-uploader--uploading">
            {i18n.t('views.pickers.assets.upload_in_progress')}
          </div>
        )}
      </div>
    )
