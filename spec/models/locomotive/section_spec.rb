    ...
    describe 'definition validation' do
      ...
      describe 'settings validation' do
        ...
        it 'requires the type property to be included in the list of available ones' do
          ...
          expect(section).to_not be_valid
          expect(section.errors[:definition]).to eq(["The property '#/settings/0/type' value \"color\" did not match one of the following values: text, image_picker, checkbox, select, url, radio, content_type, hint"])
        end
